using dnlib.DotNet;
using Hotc233.Editor.ABI;
using Hotc233.Editor.Meta;
using Hotc233.Editor.Template;
using System;
...
using UnityEditor;
using UnityEngine;
using TypeInfo = Hotc233.Editor.ABI.TypeInfo;
        ...
        private bool IsBlittable(TypeSig typeSig)
        {
            ...
            switch (typeSig.ElementType)
            {
                ...
                case ElementType.ValueType:
                {
                    ...
                    if (typeDef == null)
                    {
                        throw new Exception($"type:{typeSig} definition could not be found. Please try `hotc233/Generate/LinkXml`, then Build once to generate the AOT dll, and then regenerate the bridge function");
                    }
                    if (typeDef.IsEnum)
                    {
                        return true;
