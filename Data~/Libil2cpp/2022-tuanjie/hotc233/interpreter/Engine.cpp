...
#define POP_STACK_FRAME() 
	...
	static void SetupStackFrameInfo(const InterpFrame* frame, Il2CppStackFrameInfo& stackFrame)
	{
		...
		hotc233::metadata::InterpreterImage* interpImage = hotc233::metadata::MetadataModule::GetImage(method);
		if (!interpImage)
		{
			return;
		}

		hotc233::metadata::PDBImage* pdbImage = interpImage->GetPDBImage();
		if (!pdbImage)
		{
			return;
		}
	...
	void MachineState::SetupFramesDebugInfo(il2cpp::vm::StackFrames* stackFrames)
	{
		...
		{
			if (frame.method && hotc233::metadata::IsInterpreterImplement(frame.method))
			{
				hotc233::metadata::InterpreterImage* interpImage = hotc233::metadata::MetadataModule::GetImage(frame.method);
				if (interpImage)
				{
					hotc233::metadata::PDBImage* pdbImage = interpImage->GetPDBImage();
					if (pdbImage)
