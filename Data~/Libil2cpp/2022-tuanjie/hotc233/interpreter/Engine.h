...
namespace interpreter
{
	...
	public:
		...
		~MachineState()
		{
			if (_stackBase)
			{
				...
				HOTC233_FREE(_stackBase);
			}
			if (_frameBase)
			{
				HOTC233_FREE(_frameBase);
			}
			if (_exceptionFlowBase)
			{
				HOTC233_FREE(_exceptionFlowBase);
			}
		}
	...
	private:
		...
		void InitEvalStack()
		{
			...
			_stackBase = (StackObject*)HOTC233_MALLOC_ZERO(RuntimeConfig::GetInterpreterThreadObjectStackSize() * sizeof(StackObject));
			_stackTopIdx = 0;
		...
		void InitFrames()
		{
			_frameBase = (InterpFrame*)HOTC233_CALLOC(RuntimeConfig::GetInterpreterThreadFrameStackSize(), sizeof(InterpFrame));
		...
		{
			_exceptionFlowBase = (ExceptionFlowInfo*)HOTC233_CALLOC(RuntimeConfig::GetInterpreterThreadExceptionFlowSize(), sizeof(ExceptionFlowInfo));
