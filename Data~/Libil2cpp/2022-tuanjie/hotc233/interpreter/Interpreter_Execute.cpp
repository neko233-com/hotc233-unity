...
#pragma region arith
	...
	IL2CPP_FORCE_INLINE bool TryGetSmallI4LeafValue(StackObject* slots, uint16_t index, int32_t* value)
	{
		if (index >= 64)
		{
			return false;
		}
		*value = *(int32_t*)(void*)(slots + index);
		return true;
	}

	IL2CPP_FORCE_INLINE bool TrySetSmallI4LeafValue(StackObject* slots, uint16_t index, int32_t value)
	{
		if (index >= 64)
		{
			return false;
		}
		*(int32_t*)(void*)(slots + index) = value;
		return true;
	}

	IL2CPP_FORCE_INLINE bool TryCopySmallI4LeafSlot(StackObject* slots, uint16_t dst, uint16_t src)
	{
		if (dst >= 64 || src >= 64)
		{
			return false;
		}
		slots[dst].u64 = slots[src].u64;
		return true;
	}

	IL2CPP_FORCE_INLINE bool TryExecuteSmallI4LeafFastPath(const InterpMethodInfo* imi, StackObject* argBasePtr, void* ret)
	{
		if (!imi || !argBasePtr || !ret || imi->maxStackSize > 64 || imi->codeLength > 128 || imi->argStackObjectSize > imi->maxStackSize)
		{
			return false;
		}

		StackObject slots[64];
		for (uint32_t i = 0; i < imi->maxStackSize; i++)
		{
			slots[i].u64 = 0;
		}
		for (uint32_t i = 0; i < imi->argStackObjectSize; i++)
		{
			slots[i] = argBasePtr[i];
		}
		...
		for (uint32_t offset = 0; offset < imi->codeLength;)
		{
			...
			byte* ip = codes + offset;
			switch (op)
			{
			case HiOpcodeEnum::LdlocVarVar:
				if (!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 2), *(uint16_t*)(ip + 4)))
				{
					return false;
				}
				break;
			case HiOpcodeEnum::LdcVarConst_4:
				if (!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 2), *(int32_t*)(ip + 4)))
				{
					return false;
				}
				break;
			...
			case HiOpcodeEnum::BinOpVarVarVar_Xor_i4:
			{
				int32_t lhs;
				int32_t rhs;
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 4), &lhs) ||
					!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 6), &rhs))
				{
					return false;
				}
				int32_t value = 0;
				switch (op)
				{
				case HiOpcodeEnum::BinOpVarVarVar_Add_i4: value = lhs + rhs; break;
				case HiOpcodeEnum::BinOpVarVarVar_Sub_i4: value = lhs - rhs; break;
				case HiOpcodeEnum::BinOpVarVarVar_Mul_i4: value = lhs * rhs; break;
				case HiOpcodeEnum::BinOpVarVarVar_And_i4: value = lhs & rhs; break;
				case HiOpcodeEnum::BinOpVarVarVar_Or_i4: value = lhs | rhs; break;
				case HiOpcodeEnum::BinOpVarVarVar_Xor_i4: value = lhs ^ rhs; break;
				default: return false;
				}
				if (!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 2), value))
				{
					return false;
				}
				break;
			...
			{
				if (!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 2), *(uint16_t*)(ip + 4)) ||
					!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 6), *(int32_t*)(ip + 8)))
				{
					return false;
				}
				int32_t lhs;
				int32_t rhs;
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 14), &lhs) ||
					!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 16), &rhs))
				{
					return false;
				}
				int32_t value = op == HiOpcodeEnum::LdlocVarVar_LdcVarConst_4_BinOpAdd_i4 ? lhs + rhs : lhs * rhs;
				if (!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 12), value))
				{
					return false;
				}
				break;
			...
			case HiOpcodeEnum::LdlocVarVar_LdcVarConst_4_BinOpMul_i4_RetVar_ret_4:
			{
				...
				if (!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 12), lhs * rhs))
				{
					return false;
				}
				int32_t retValue;
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 18), &retValue))
				{
					return false;
				}
				*(int32_t*)ret = retValue;
				return true;
			}
			case HiOpcodeEnum::LdlocVarVar_LdcVarConst_4_BinOpAdd_i4_LdlocVarVar:
			{
				...
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 14), &lhs) ||
					!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 16), &rhs) ||
					!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 12), lhs + rhs) ||
					!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 18), *(uint16_t*)(ip + 20)))
				{
					return false;
				}
				break;
			...
			case HiOpcodeEnum::BinOpVarVarVar_Add_i4_LdcVarConst_4:
			{
				...
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 4), &lhs) ||
					!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 6), &rhs) ||
					!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 2), lhs + rhs) ||
					!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 8), *(int32_t*)(ip + 12)))
				{
					return false;
				}
				break;
			...
			case HiOpcodeEnum::BinOpVarVarVar_Add_i4_LdlocVarVar:
			{
				...
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 4), &lhs) ||
					!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 6), &rhs) ||
					!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 2), lhs + rhs) ||
					!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 8), *(uint16_t*)(ip + 10)))
				{
					return false;
				}
				break;
			...
			case HiOpcodeEnum::BinOpVarVarVar_Add_i4_LdlocVarVar_LdlocVarVar_LdlocVarVar:
			{
				...
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 4), &lhs) ||
					...
					!TrySetSmallI4LeafValue(slots, *(uint16_t*)(ip + 2), lhs + rhs) ||
					!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 8), *(uint16_t*)(ip + 10)) ||
					!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 12), *(uint16_t*)(ip + 14)) ||
					!TryCopySmallI4LeafSlot(slots, *(uint16_t*)(ip + 16), *(uint16_t*)(ip + 18)))
				{
					return false;
				}
				break;
			...
			{
				int32_t retValue;
				if (!TryGetSmallI4LeafValue(slots, *(uint16_t*)(ip + 2), &retValue))
				{
					return false;
				}
				*(int32_t*)ret = retValue;
			...
			offset += g_instructionSizes[(int)op];
		}
		return false;
	}
	...
	IL2CPP_FORCE_INLINE bool TryExecuteHotc233FastPath(const InterpMethodInfo* imi, StackObject* localVarBase, void* ret)
	{
		...
		case Hotc233FastPath_SmallI4Leaf:
			return TryExecuteSmallI4LeafFastPath(imi, localVarBase, ret);
