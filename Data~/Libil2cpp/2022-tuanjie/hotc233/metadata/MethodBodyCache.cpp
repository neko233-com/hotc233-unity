...
namespace metadata
{
	...
	static MethodBodyCacheInfo* GetOrInitMethodBodyCache(hotc233::metadata::Image* image, uint32_t token)
	{
		...
		MethodBodyCacheInfo* ci = (MethodBodyCacheInfo*)HOTC233_MALLOC_ZERO(sizeof(MethodBodyCacheInfo));
		*ci = { methodBody, s_methodBodyCacheVersion, 0, InlineMode::None};
		s_methodBodyCache[key] = ci;
		return ci;
	}

	MethodBody* MethodBodyCache::GetMethodBody(hotc233::metadata::Image* image, uint32_t token)
	{
