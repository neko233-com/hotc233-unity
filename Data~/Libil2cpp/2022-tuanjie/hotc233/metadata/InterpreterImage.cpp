...
namespace metadata
{
...
#else
		Il2CppImageGlobalMetadata* metadataImage = (Il2CppImageGlobalMetadata*)HOTC233_METADATA_MALLOC(sizeof(Il2CppImageGlobalMetadata));
	...
	void InterpreterImage::ComputeBlittable(Il2CppTypeDefinition* def, std::vector<bool>& computFlags)
	{
		...
		if (type->type == IL2CPP_TYPE_VALUETYPE)
		{
			...
			for (int i = 0; i < def->field_count; i++)
			{
				...
				if (!hotc233::metadata::IsInstanceField(fieldType))
				{
					continue;
				}
	...
	{
		hotc233::interpreter::ExecutingInterpImageScope scope(hotc233::interpreter::InterpreterModule::GetCurrentThreadMachineState(), this->_il2cppImage);
		...
		void* resultData = HOTC233_MALLOC(_il2cppFormatCustomDataBlob.Size());
	...
	CustomAttributesCache* InterpreterImage::GenerateCustomAttributesCacheInternal(CustomAttributeIndex index)
	{
		...
		for (int32_t i = 0; i < count; i++)
		{
			...
			HOTC233_SET_WRITE_BARRIER((void**)cache->attributes + i);
		}
	...
	void InterpreterImage::InitImplMaps()
	{
		...
		for (uint32_t rid = 1; rid <= implMapTb.rowNum; rid++)
		{
			...
			uint32_t memberForwardedToken = hotc233::metadata::ConvertMemberForwardedToken2Token(implMap.memberForwarded);
	...
	void InterpreterImage::ComputeVTable(TypeDefinitionDetail* tdd)
	{
		...
		{
			tdd->vtable = (VirtualMethodImpl*)HOTC233_METADATA_CALLOC(vms.size(), sizeof(VirtualMethodImpl));
	...
	Il2CppMethodPointer InterpreterImage::GetAdjustorThunk(uint32_t token)
	{
		...
		return IsInstanceMethod(methodDef) ? hotc233::interpreter::InterpreterModule::GetAdjustThunkMethodPointer(methodDef) : nullptr;
	}
	...
	Il2CppMethodPointer InterpreterImage::GetMethodPointer(uint32_t token)
	{
		...
		IL2CPP_ASSERT(methodIndex < (uint32_t)_methodDefines.size());
		const Il2CppMethodDefinition* methodDef = &_methodDefines[methodIndex];
		return hotc233::interpreter::InterpreterModule::GetMethodPointer(methodDef);
	}
	...
	InvokerMethod InterpreterImage::GetMethodInvoker(uint32_t token)
	{
		...
		IL2CPP_ASSERT(methodIndex < (uint32_t)_methodDefines.size());
		const Il2CppMethodDefinition* methodDef = &_methodDefines[methodIndex];
		return hotc233::interpreter::InterpreterModule::GetMethodInvoker(methodDef);
	}
	...
	void InterpreterImage::ReadFixedArg(BlobReader& reader, const Il2CppType* argType, void* data)
	{
		...
		case IL2CPP_TYPE_SZARRAY:
		{
			...
			HOTC233_SET_WRITE_BARRIER((void**)data);
			break;
		}
		case IL2CPP_TYPE_STRING:
		{
