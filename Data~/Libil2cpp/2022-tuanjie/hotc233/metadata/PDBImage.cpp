...
namespace metadata
{
	...
	const PDBImage::SymbolDocumentData* PDBImage::GetDocument(uint32_t documentToken)
	{
		...
		SymbolDocumentData* documentData = new (HOTC233_MALLOC_ZERO(sizeof(SymbolDocumentData))) SymbolDocumentData();
	...
	void PDBImage::SetupStackFrameInfo(const MethodInfo* method, const void* ip, Il2CppStackFrameInfo& stackFrame)
	{
		...
		const hotc233::interpreter::InterpMethodInfo* imi = (const hotc233::interpreter::InterpMethodInfo*)method->interpData;
	...
	void PDBImage::SetMethodDebugInfo(const MethodInfo* method, const il2cpp::utils::dynamic_array<ILMapper>& ilMapper)
	{
		...
		SymbolMethodInfoData* methodInfoData = new (HOTC233_MALLOC_ZERO(sizeof(SymbolMethodInfoData))) SymbolMethodInfoData();
	...
	PDBImage::SymbolMethodDefData* PDBImage::GetMethodDataFromCache(uint32_t methodToken)
	{
		...
		uint32_t rowIndex = hotc233::metadata::DecodeTokenRowIndex(methodToken);
		if (rowIndex == 0 || rowIndex > tableMeta.rowNum)
		{
			return nullptr;
		}
		...
		SymbolMethodDefData* methodData = new (HOTC233_MALLOC_ZERO(sizeof(SymbolMethodDefData))) SymbolMethodDefData();
